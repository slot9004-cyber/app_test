...
class SnapDnnCppDroidBenchmark(object):
    ...
    def __init__(self, exe_dir, dep_lib_dir, exe_name, model_dir, container_name,
                 ...
                 platform_option,  ubfloat_output, host_rootpath, cdsp, cache=None, udo=None):
    ...
    def __create_script(self):
        cmds = ['export LD_LIBRARY_PATH=' + self._dep_lib_dir + ':$LD_LIBRARY_PATH',
                ...
                ';/system/lib/rfsa/adsp;/usr/lib/rfsa/adsp;/system/vendor/lib/rfsa/adsp;/dsp;/etc/images/dsp;\"',
                'cd ' + self._model_dir,
        ...
        if self.platform_option or self.cdsp == "1":
            if self.cdsp == "1":
                cmds.append('export ADSP_LIBRARY_PATH="/usr/lib/dsp/cdsp;/usr/lib/dsp/cdsp1;$ADSP_LIBRARY_PATH"')
                cmds.append("export LD_LIBRARY_PATH=$PWD")
                run_cmd += ' --platform_options="deviceId:1"'
            else:
